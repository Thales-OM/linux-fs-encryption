...
static int encfs_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi) {
    ...
    if (S_ISREG(stbuf->st_mode)) {
        if (stbuf->st_size >= (off_t)NONCE_BYTES)
...
static int encfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        snprintf(raw_path, PATH_MAX, "%s/%s", raw, de->d_name);
...
static int encfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    struct stat st;
    fstat(fi->fh, &st);
    if (st.st_size < (off_t)NONCE_BYTES) return -EIO;
    ...
    size_t avail = mlen > (unsigned long long)offset ? mlen - offset : 0;
...
static int encfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    struct stat st;
    fstat(fi->fh, &st);
    unsigned long long old_plain_len = (st.st_size > (off_t)NONCE_BYTES) ? st.st_size - NONCE_BYTES : 0;
    unsigned char *old_plain = calloc(old_plain_len ? old_plain_len : 1, 1);
    
    if (old_plain_len > 0) {
        unsigned char *raw = malloc(st.st_size);
        pread(fi->fh, raw, st.st_size, 0);
        if (crypto_secretbox_open_easy(old_plain, raw + NONCE_BYTES, old_plain_len, raw, KEY) != 0) {
            free(raw); free(old_plain);
            return -EIO;
        }
        free(raw);
    }
    ...
    if (new_len < old_plain_len) new_len = old_plain_len;
